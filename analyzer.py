...
{"proposal": "提案文本文", "deadline_days": 納期日数(整数)}

納期日数の目安：
- 小規模修正・バナー（〜5万円）: 7〜14日
- LP・コーポレートサイト（5〜30万円）: 21〜30日
- EC構築・Shopify（30〜100万円）: 30〜60日
- 大規模開発（100万円〜）: 60〜90日
- 月額常駐案件: 30日"""

PROPOSAL_USER_TEMPLATE = """以下のランサーズ案件に対する提案文と納期日数を返してください。
...
def generate_proposal(project: dict, profile: dict) -> dict:
    """Generate a Japanese proposal message for a single project.
    Returns dict with 'proposal' (str) and 'deadline_days' (int).
    """
    ...
    try:
        ...
        return {
            ...
            "deadline_days": int(result.get("deadline_days", 30)),
        }
    except (json.JSONDecodeError, ValueError):
        # Fallback: treat entire response as proposal text
        return {"proposal": raw, "deadline_days": 30}
